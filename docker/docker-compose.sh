...
##### Database server #####
###########################
docker_path="/opt/mangos3/database";
...
user_command "${repo_path}" "SOURCE ${docker_path}/Character/Setup/characterLoadDB.sql" "character4"
...
then
    ...
    do
        ...
        root_command "${repo_path}" "SOURCE ${docker_path}/Character/Updates/Rel21/${sql}" "character4";
    done
fi
...
then
    ...
    do
        echo "Importing Character update ${sql}";
        root_command "${repo_path}" "SOURCE ${docker_path}/Character/Updates/Rel22/${sql}" "character4";
    done
fi
# find "${repo_path}/Character/Updates/Rel21" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "character4"' _ {} \;

##########################
##### World Database #####
##########################
root_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/mangosdCreateDB.sql" "" || echo "Realm DB already exists";
user_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/mangosdLoadDB.sql" "mangos4"
...
then
    ...
    do
        ...
        root_command "${repo_path}" "SOURCE ${docker_path}/World/Setup/FullDB/${sql}" "mangos4";
    done
fi
# find "${repo_path}/World/Setup/FullDB" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "mangos4"' _ {} \;
...
then
    ...
    do
        ...
        root_command "${repo_path}" "SOURCE ${docker_path}/World/Updates/Rel21/${sql}" "mangos4";
    done
fi
...
then
    ...
    do
        echo "Importing World Update ${sql}";
        root_command "${repo_path}" "SOURCE ${docker_path}/World/Updates/Rel22/${sql}" "mangos4";
    done
fi
# find "${repo_path}/World/Updates/Rel21" -maxdepth 1 -iname '*.sql' -exec bash -c 'root_command "'${repo_path}'" "SOURCE $1" "mangos4"' _ {} \;
